...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static void HandleLedBlink(uint8_t count)
{
...
static void HandleButtonPress(void)
{
  ...
  HAL_UART_Transmit(&huart2, &tx_byte, 1, HAL_MAX_DELAY);

  if (HAL_UART_Receive(&huart2, &rx_byte, 1, UART_RX_TIMEOUT_MS) == HAL_OK)
  {
    if (rx_byte == UART_RSP_OK)
    {
      HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET);
    }
    else if (rx_byte == UART_RSP_NG)
    {
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  if (GPIO_Pin == B1_Pin)
  {
    ...
    if ((now - last_button_tick) >= BTN_DEBOUNCE_MS)
    {
